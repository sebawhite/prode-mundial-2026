...
1. **Identity Integrity**: Users can only create or update their own user profile document (`/users/{userId}` where `{userId} == request.auth.uid`) and their own matches predictions (`/predictions/{uid_matchId}` which must have `userId` matched to auth ID).
2. **Read-Isolations**:
...
3. **Write-Isolations**:
   - Predictive modifications on matches can only be made until the `groupStageDeadline` timestamp passes.
   - Once past the deadline, predictions are completely locked for edits.
   - **Finished Match Invariant**: Predictions are strictly locked for any match that is already finished (`isFinished == true`), regardless of whether the group stage deadline has passed or not.
...
## 2. The "Dirty Dozen" Payloads (Authenticated / Malicious Actions)
...
4. **Payload 4: Finished Match Predict Bypass**
   - Action: Create or update prediction for a match where `isFinished` is `true`, even if the global deadline is still in the future.
   - Expected Result: `PERMISSION_DENIED`
...
6. **Payload 6: Unauthorized Match Score tampering**
   ...
   - Expected Result: `PERMISSION_DENIED`
7. **Payload 8: Self-Validating Payments**
