...
export const Welcome: React.FC<WelcomeProps> = ({ onNavigate }) => {
  ...
  const zanzibarPercent = Math.round((1 - config.poolPercent) * 100);
  const poolPercentVal = Math.round(config.poolPercent * 100);
  const zanzibarAmount = config.buyInAmount * (1 - config.poolPercent);
  const poolAmount = config.buyInAmount * config.poolPercent;
      ...
      <div className="bg-brand-card retro-border retro-shadow p-6 rounded-lg text-brand-ink mb-6 relative">
        ...
        <div className="bg-brand-bg/80 border-2 border-brand-ink p-3 rounded-md mb-4 text-left space-y-2">
          ...
          <p className="font-sans text-xs md:text-sm italic text-brand-ink-muted border-t border-brand-ink/10 pt-2">
            🌍 <strong>{zanzibarPercent}% Solidario:</strong> De cada inscripción de ${config.buyInAmount.toLocaleString('es-AR')}, <strong>${zanzibarAmount.toLocaleString('es-AR')} se donan de forma directa</strong> para comprar bancos de madera para la escuela <strong className="text-brand-blue font-bold">Wonderful School</strong> en Zanzíbar, Tanzania 🇹🇿.
          </p>
          <p className="font-sans text-xs md:text-sm italic text-brand-ink-muted border-t border-brand-ink/10 pt-2">
            🎁 <strong>{poolPercentVal}% Pozo de Premios:</strong> Los restantes ${poolAmount.toLocaleString('es-AR')} se acumulan transparentemente para repartirse íntegramente entre el Top 3 final (1°: 60%, 2°: 25%, 3°: 15%).
          </p>
        </div>
        ...
        <div className="border-t border-brand-ink/20 pt-3 text-left font-mono text-xs text-brand-ink space-y-2">
          <div className="flex justify-between">
            ...
            <span className="font-bold">{config.buyInAmount.toLocaleString('es-AR')} Pesos</span>
          </div>
          <div className="flex justify-between">
            <span>🌍 Causa Solidaria:</span>
            <span className="font-bold text-brand-blue">{zanzibarPercent}% Donación Zanzíbar 🇹🇿</span>
          </div>
          <div className="flex justify-between">
            <span>🏆 Reparto del Pozo:</span>
            <span className="font-bold">1°: 60% | 2°: 25% | 3°: 15% del restante {poolPercentVal}%</span>
          </div>
          <div className="flex justify-between">
